...
func transaction(nonce uint64, gaslimit uint64, key *sm2.PrivateKey) *types.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *sm2.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HomesteadSigner{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *sm2.PrivateKey, bytes uint64) *types.Transaction {
...
func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *sm2.PrivateKey) *types.Transaction {
...
func setupTxPool() (*TxPool, *sm2.PrivateKey) {
	return setupTxPoolWithConfig(params.TestChainConfig)
}

func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *sm2.PrivateKey) {
...
func TestTransactionDropping(t *testing.T) {
	...
	account := gmsm.PubkeyToAddress(key.PublicKey)
...
func TestTransactionPostponing(t *testing.T) {
	...
	keys := make([]*sm2.PrivateKey, 2)
	accs := make([]common.Address, len(keys))

	for i := 0; i < len(keys); i++ {
		keys[i], _ = gmsm.GenerateKey()
		accs[i] = gmsm.PubkeyToAddress(keys[i].PublicKey)

		testAddBalance(pool, gmsm.PubkeyToAddress(keys[i].PublicKey), big.NewInt(50100))
	}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	keys := make([]*sm2.PrivateKey, 5)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = gmsm.GenerateKey()
		testAddBalance(pool, gmsm.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000))
	}
	...
	if nolocals {
		...
		if queued := pool.queue[gmsm.PubkeyToAddress(local.PublicKey)].Len(); uint64(queued) != 3*config.GlobalQueue {
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	local, _ := gmsm.GenerateKey()
	remote, _ := gmsm.GenerateKey()

	testAddBalance(pool, gmsm.PubkeyToAddress(local.PublicKey), big.NewInt(1000000000))
	testAddBalance(pool, gmsm.PubkeyToAddress(remote.PublicKey), big.NewInt(1000000000))
	...
	statedb.SetNonce(gmsm.PubkeyToAddress(remote.PublicKey), 2)
	statedb.SetNonce(gmsm.PubkeyToAddress(local.PublicKey), 2)
	<-pool.requestReset(nil, nil)
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	// Create a number of test accounts and fund them
	keys := make([]*sm2.PrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = gmsm.GenerateKey()
		testAddBalance(pool, gmsm.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000*1000000))
	}
...
func TestDualHeapEviction(t *testing.T) {
	...
	add := func(urgent bool) {
		for i := 0; i < 20; i++ {
			...
			key, _ := gmsm.GenerateKey()
			testAddBalance(pool, gmsm.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000000))
			if urgent {
...
func TestTransactionDeduplication(t *testing.T) {
	...
	key, _ := gmsm.GenerateKey()
	testAddBalance(pool, gmsm.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	statedb.SetNonce(gmsm.PubkeyToAddress(local.PublicKey), 1)
	blockchain = &testBlockChain{1000000, statedb, new(event.Feed)}
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	remoteKey, _ := gmsm.GenerateKey()
	remoteAddr := gmsm.PubkeyToAddress(remoteKey.PublicKey)
