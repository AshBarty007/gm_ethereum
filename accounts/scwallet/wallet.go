...
func (w *Wallet) SignDataWithPassphrase(account accounts.Account, passphrase, mimeType string, data []byte) ([]byte, error) {
	return w.signHashWithPassphrase(account, passphrase, gmsm.SM3(data))
}
...
func (w *Wallet) SignTextWithPassphrase(account accounts.Account, passphrase string, text []byte) ([]byte, error) {
	return w.signHashWithPassphrase(account, passphrase, gmsm.SM3(accounts.TextHash(text)))
}
...
func (s *Session) initialize(seed []byte) error {
	...
	key, err := gmsm.ToSM2(seed[:32])
	if err != nil {
		return err
	}

	id := initializeData{}
	id.PublicKey = gmsm.FromSM2Pub(&key.PublicKey)
...
func (s *Session) derive(path accounts.DerivationPath) (accounts.Account, error) {
	...
	return s.Wallet.makeAccount(gmsm.PubkeyToAddress(*pub), path), nil
}
